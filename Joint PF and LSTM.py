# -*- coding: utf-8 -*-
"""
Created on Sun Dec  1 10:56:37 2019
...
###############################################################################MC-dropout模拟
regress=load_model('B0005_cycle80_training model_1')                          #载入训练的LSTM模型
...
##########联合预测
SP=79           #联合预测起点循环数为80，python中计算向前一位取79  
...
for k in range(1,N):
    ...
    for i in range(0,M):
        ...
        if k>=SP:
            ...
            Predict_k=regress.predict(X_input)                                #LSTM one-step ahead prediction
            Z[0,k]=sc.inverse_transform(Predict_k).reshape(1,)
